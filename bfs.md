...
　　从左上到右下的状态转移方程为`dp[i][j] = min(dp[i][j], dp[i-1][j] + 1, dp[i][j-1] + 1)`。  　　
...
　　需要注意的是相邻数字的处理，由于`Python3`取模的结果一定是正数，也就是`-1 % 10 = 9`，所以可以用取模来简化代码。四位数字中的任意一为`±1`然后`%10`就可以得到下一种可能性。  
